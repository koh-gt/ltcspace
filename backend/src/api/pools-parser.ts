    ...
    for (const pool of this.miningPools) {
      ...
      if (!poolDB) {
        ...
        if (poolDB.name !== pool.name) {
          ...
          logger.warn(`Renaming ${poolDB.name} mining pool to ${pool.name}. Slug has been updated. Maybe you want to make a redirection from 'http://mempool.ferritecoin.org/mining/pool/${poolDB.slug}' to 'https://mempool.ferritecoin.org/mining/pool/${newSlug}`);
